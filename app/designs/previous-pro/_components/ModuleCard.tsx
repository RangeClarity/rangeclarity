...
export default function ModuleCard({ m }: { m: ModuleV2 }) {
  const vars = {
    ["--cc" as string]: m.c,
    ["--cc2" as string]: m.c2,
  ...
  return (
    <article className={s.modCard} style={vars}>
      ...
      <div className={s.modMeta}>
        <div className={s.metaItem}><span className={s.metaKey}>Shows</span><span className={s.metaVal}>{m.shows}</span></div>
        <div className={s.metaItem}><span className={s.metaKey}>Avoids</span><span className={s.metaVal}>{m.avoids}</span></div>
        <div className={s.metaItem}><span className={s.metaKey}>Best for</span><span className={s.metaVal}>{m.bestFor}</span></div>
      </div>
    </article>
  );
}
